...
auto crypto_currency::daily_fluctuations() -> bool {
    ...
    circulating_sup +=
        random_sup; // missing formula to change circulating supply
...
int main(int argc, char *argv[]) {
    ...
    Element doc1 = hbox({vbox({
                             text(L"middler") | border | flex,
                             text(L"middle") | border | flex,
                         }) | border |
                             flex,
                         text(L"OOOOO") | border | flex});
    // Define the document

    auto screen = Screen::Create(Dimension::Full(),   // Width
                                 Dimension::Fit(doc1) // Height
    );
    Render(screen, doc1);
    screen.Print();
